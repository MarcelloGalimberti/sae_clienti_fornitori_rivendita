...
def tabella_semaforo_numerica(df_num, col_marg, soglia_bassa, soglia_alta):
    """
    Variante di tabella_semaforo per tabelle riepilogative sortable.
    Mantiene i valori come float nel DataFrame sottostante e usa
    Styler.format() per la visualizzazione in formato italiano,
    così Streamlit ordina numericamente (non alfabeticamente).
    """
    colori_marg = df_num[col_marg].apply(
        lambda x: C.get(semaforo(x, soglia_bassa, soglia_alta), C['grigio'])
    ).tolist()
    ...
    for col in df_num.columns:
        if not pd.api.types.is_numeric_dtype(df_num[col]):
            continue
        col_l = col.lower()
        if ('(€)' in col or col_l in [
                'utile/perdita', 'fatturato', 'costo acquisto',
                'prezzo finale_cliente', 'prezzo finale_fornitore integrato',
                'costo medio fornitore', 'prezzo medio cliente',
                'prezzo medio acquisto', 'fatturato_vendita', 'costo_acquisto']):
            fmt[col] = lambda x: fmt_eur(x, 0) if not pd.isna(x) else 'N/D'
        elif '(%)' in col or col_l in ['mark-up', 'marginalità', 'marginalita']:
            fmt[col] = lambda x: fmt_pct(x, 1) if not pd.isna(x) else 'N/D'
        else:
            fmt[col] = lambda x: fmt_num(x, 0) if not pd.isna(x) else 'N/D'

    return (
        df_num.style
        .apply(_highlight, subset=[col_marg], axis=0)
...
with st.sidebar:
    ...
    st.divider()
    st.caption('v5.0 — SAE Scientifica')
...
with tab_cli:
    ...
    if df_mup_cli.empty:
        ...
        with col_tbl_cli:
            st.dataframe(
                tabella_semaforo_numerica(df_mup_cli, 'Mark-Up (%)', soglia_bassa, soglia_alta),
                use_container_width=True, hide_index=True
            )
        with col_bar_cli:
...
with tab_forn:
    ...
    if df_mup_forn.empty:
        ...
        with col_tbl_forn:
            st.dataframe(
                tabella_semaforo_numerica(df_mup_forn, 'Mark-Up (%)', soglia_bassa, soglia_alta),
                use_container_width=True, hide_index=True
            )
        with col_bar_forn:
