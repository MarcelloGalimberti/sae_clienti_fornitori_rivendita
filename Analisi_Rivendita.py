# =============================================================================
# Analisi Marginalità Rivendita — App Unificata Clienti & Fornitori
...
# ── PAGE CONFIG ───────────────────────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="Analisi Marginalità Rivendita")
...
COLONNE = [
    'Codice anagrafica', 'Cliente/Fornitore', 'Codice articolo',
...
def formatta_df(df):
    ...
    for col in df_out.columns:
        ...
        elif '(%)' in col or col_l in ['marginalità', 'marginalita']:
...
def calcola_marginalita_forn(df_calc):
    ...
    - Marginalità per articolo: (prezzo_medio_cli / costo_medio_forn - 1) × 100
...
def marginalita_complessiva_forn(df_art):
    """
    Marginalità aggregata (logica Fornitori 2): fatturato_vendita / costo_acquisto - 1
...
def trend_mensile_forn(df_periodo, mask=None):
    ...
    for periodo in sorted(df['Anno-Mese'].unique()):
        ...
        rows.append({
            ...
            'Periodo_ord':         periodo,
            'Marginalità (%)':     round(marg, 2) if not np.isnan(marg) else np.nan,
...
def fig_trend(df_t, titolo, soglia_bassa, soglia_alta):
    ...
    marg_vals = df_t['Marginalità (%)'].tolist()
    ...
    fig.update_layout(
        ...
        yaxis=dict(title='€', tickformat=',.0f',
                   titlefont=dict(size=14), tickfont=dict(size=13)),
        yaxis2=dict(title='Marginalità (%)', overlaying='y', side='right',
                    tickformat='.1f',
                    titlefont=dict(size=14), tickfont=dict(size=13)),
...
def fig_distribuzione(df_art, marg_comp):
    vals = df_art['Marginalità'].dropna().values
    if len(vals) < 2:
        return None
    fig = ff.create_distplot([vals], ['Marginalità'], bin_size=25,
                             colors=[C['verde']], show_hist=False)
    fig.add_vline(x=marg_comp, line_dash='dash', line_color=C['rosso'], line_width=2,
                  annotation_text=f'  Complessiva: {marg_comp:.1f}%',
    ...
    fig.update_layout(
        title=dict(text='Distribuzione Marginalità per articolo',
                   font=dict(size=15, color=C['navy'])),
        height=460, xaxis_title='Marginalità (%)',
...
def fig_scatter_marg(df_art):
    df_p = df_art.dropna(subset=['Marginalità', 'Prezzo finale_Cliente']).copy()
    if df_p.empty:
        return None
    df_p['Qty_scaled'] = np.sqrt(df_p['Quantità_Cliente'].fillna(1))
    fig = px.scatter(
        df_p, x='Marginalità', y='Prezzo finale_Cliente',
        ...
        title='Marginalità vs Fatturato (asse Y logaritmico)',
...
def fig_pareto_clienti(df_periodo):
    ...
    fig.update_layout(
        ...
        yaxis=dict(title='Fatturato (€)', tickformat=',.0f',
                   titlefont=dict(size=13), tickfont=dict(size=12)),
...
def fig_treemap_fornitori(df_dati, cod_forn, soglia_bassa, soglia_alta):
    ...
    for (cod_c, nome_c, macro), grp in df_cli.groupby(
        ...
        rows.append({
            ...
            'Marginalità (%)':  round(mg, 1) if not np.isnan(mg) else np.nan,
        })
    ...
    df_tree_clean = df_tree.dropna(subset=['Marginalità (%)']).copy()
    ...
    fig = px.treemap(
        ...
        title='Panoramica Cliente > Macroprogetto  |  colore = Marginalità %  |  dimensione = Fatturato',
        hover_data={'Fatturato (€)': True, 'Costo (€)': True,
                    'Utile/Perdita (€)': True, 'Marginalità (%)': True},
        height=600,
    )
    fig.update_traces(
        ...
        hovertemplate=(
            ...
            'Marginalità: %{customdata[3]:.1f}%<extra></extra>'
        )
    )
...
def fig_bar_clienti_marginalita(df_tree, soglia_bassa, soglia_alta):
    ...
    df_cli = (
        df_tree.dropna(subset=['Marginalità (%)'])
        .groupby('Cliente')
        ...
        .reset_index()
        .sort_values('Marginalità (%)', ascending=True)
    )
    colors = [C.get(semaforo(v, soglia_bassa, soglia_alta), C['grigio'])
              for v in df_cli['Marginalità (%)']]
    fig = go.Figure(go.Bar(
        y=df_cli['Cliente'],
        x=df_cli['Marginalità (%)'],
        orientation='h',
        marker_color=colors,
        text=[f"{v:.1f}%" for v in df_cli['Marginalità (%)']],
        textposition='outside',
        textfont=dict(size=13),
        hovertemplate='<b>%{y}</b><br>Marginalità: %{x:.1f}%<extra></extra>'
    ))
    fig.add_vline(x=soglia_bassa, line_dash='dot', line_color=C['arancio'],
                  line_width=1.5,
                  annotation_text=f'  {soglia_bassa}%',
                  annotation_font=dict(color=C['arancio']))
    fig.add_vline(x=soglia_alta, line_dash='dot', line_color=C['verde_scuro'],
                  line_width=1.5,
                  annotation_text=f'  {soglia_alta}%',
                  annotation_font=dict(color=C['verde_scuro']))
    fig.update_layout(
        title=dict(text='Marginalità per Cliente (dal fornitore selezionato)',
                   font=dict(size=15, color=C['navy'])),
        xaxis_title='Marginalità (%)',
...
def genera_pdf(titolo, periodo_str, kpi_dict: dict,
    ...
    pdf.set_text_color(160, 160, 160)
    pdf.cell(0, 5, 'SAE Scientifica | ADI Business Consulting - Analisi Marginalita Rivendita',
             align='C')
...
def mostra_kpi_row(fat, cost, utile, marg, soglia_bassa, soglia_alta, prefisso=''):
    ...
    with col4:  kpi_card(f'{prefisso}Marginalità complessiva', fmt_pct(marg),
                         semaforo(marg, soglia_bassa, soglia_alta))
...
def mostra_analisi_articoli(df_art, marg_comp, soglia_bassa, soglia_alta,
    ...
    df_plot = df_art.dropna(subset=['Marginalità']).copy()
    if len(df_plot) >= 2:
    ...
    num = st.slider('N articoli top / bottom:', 1, 20, 5, key=f'{key_prefix}_slider')
    col_t, col_b = st.columns(2)
    with col_t:
        st.markdown(f'**Top {num} per marginalità**')
        df_top = df_art.dropna(subset=['Marginalità']).nlargest(num, 'Marginalità')[cols_vis].copy()
        df_top['Marginalità'] = df_top['Marginalità'].round(1)
        st.dataframe(tabella_semaforo(df_top, 'Marginalità', soglia_bassa, soglia_alta),
                     hide_index=True, use_container_width=True)
    with col_b:
        st.markdown(f'**Bottom {num} per marginalità**')
        df_bot = df_art.dropna(subset=['Marginalità']).nsmallest(num, 'Marginalità')[cols_vis].copy()
        df_bot['Marginalità'] = df_bot['Marginalità'].round(1)
        st.dataframe(tabella_semaforo(df_bot, 'Marginalità', soglia_bassa, soglia_alta),
                     hide_index=True, use_container_width=True)
    ...
    df_up['Marginalità'] = df_up['Marginalità'].round(1)
    st.dataframe(tabella_semaforo(df_up, 'Marginalità', soglia_bassa, soglia_alta),
...
with col_titolo:
    st.markdown(
        ...
        f"margin-top:10px'>Analisi Marginalità Rivendita</h2>",
        unsafe_allow_html=True
    )
with col_logo_dx:
...
with st.sidebar:
    ...
    st.subheader('Soglie marginalità')
    soglia_bassa = st.number_input('Soglia bassa (%)', min_value=0,
                                   max_value=100, value=10, step=1)
    soglia_alta  = st.number_input('Soglia alta (%)',  min_value=0,
    ...
    st.markdown(
        ...
        f"&nbsp;|&nbsp;  <span style='color:{C['rosso']}'>Rosso</span> &lt; {soglia_bassa}%</small>",
        unsafe_allow_html=True
    )
    st.divider()
    st.caption('v1.0 — SAE Scientifica')
...
with st.expander('Andamento mensile aggregato', expanded=True):
    ...
    st.plotly_chart(
        fig_trend(df_trend_all,
                  f'Marginalità mensile — tutti i clienti/fornitori | {periodo_label}',
    ...
    if cliente_sel == '— Seleziona Cliente —':
        ...
        with st.expander(f'Andamento mensile — {nome_cli}', expanded=True):
            ...
            fig_tc = fig_trend(df_trend_cli,
                               f'Marginalità mensile — {nome_cli}',
                               soglia_bassa, soglia_alta)
        ...
        st.subheader('Marginalità per Macroprogetto', divider='green')
        ...
        if not df_macro_tab.empty:
            st.dataframe(
                tabella_semaforo(df_macro_tab, 'Marginalità (%)', soglia_bassa, soglia_alta),
                use_container_width=True, hide_index=True
            )
        ...
        if macro_sel:
            # ── Tabella Progetti ──────────────────────────────────────────────
            st.subheader('Marginalità per Progetto', divider='green')
            proj_list = sorted(
            ...
            for proj in proj_list:
                ...
                rows_proj.append({
                    ...
                    'Marginalità (%)':   round(mg_p, 1) if not np.isnan(mg_p) else np.nan,
                })

            df_proj_tab = pd.DataFrame(rows_proj)
            if not df_proj_tab.empty:
                st.dataframe(
                    tabella_semaforo(df_proj_tab, 'Marginalità (%)', soglia_bassa, soglia_alta),
                    use_container_width=True, hide_index=True
                )
            ...
            if proj_sel:
                ...
                st.markdown(
                    ...
                    f"Costo: {fmt_eur(cost_s)}  |  Marginalità: **{fmt_pct(marg_s)}**"
                )
                ...
                kpi_df = pd.DataFrame([{
                    ...
                    'Marginalità (%)':   round(marg_cli, 1) if not np.isnan(marg_cli) else None,
                }])
                with col_x1:
                    xl = to_excel({
                        'KPI':           kpi_df,
                        ...
                        'Trend mensile': df_trend_cli[['Periodo', 'Marginalità (%)',
                                                        'Fatturato', 'Costo Acquisto']],
                    })
                    st.download_button(
                        'Download Excel',
                        data=xl,
                        file_name=f'Marginalita_{nome_cli.replace(" ","_")}'
                    ...
                    if st.button('Genera PDF', key='pdf_cli_btn'):
                        with st.spinner('Generazione PDF...'):
                            kpi_pdf = {
                                ...
                                'Marginalità':   (fmt_pct(marg_cli),
                                                  semaforo(marg_cli, soglia_bassa, soglia_alta)),
                            }
                            pdf_b = genera_pdf(
                                titolo=f'Analisi Marginalità — {nome_cli}',
                                periodo_str=periodo_label,
                                kpi_dict=kpi_pdf,
                                df_riepilogo=df_macro_tab,
                                df_articoli=df_art_sel[cols_vis]
                                    .sort_values('Marginalità', ascending=False)
                                    .head(20).round(2),
...
with tab_forn:
    ...
    if fornitore_sel == '— Seleziona Fornitore —':
        ...
        with col4: kpi_card('Marginalità',           fmt_pct(marg_f),
                            semaforo(marg_f, soglia_bassa, soglia_alta))
        ...
        with st.expander(f'Andamento mensile — {nome_forn}', expanded=True):
            ...
            fig_tf = fig_trend_forn(df_trend_forn,
                                    f'Marginalità mensile — {nome_forn}',
                                    soglia_bassa, soglia_alta)
        ...
        if fig_tree is not None:
            ...
            if not df_tree.empty:
                ...
                with st.expander('Tabella riepilogativa Cliente x Macroprogetto'):
                    st.dataframe(
                        tabella_semaforo(df_tree.sort_values('Marginalità (%)', ascending=True),
                                         'Marginalità (%)', soglia_bassa, soglia_alta),
                        use_container_width=True, hide_index=True
                    )
        else:
        ...
        if not clienti_del_forn:
            ...
            if cli_drill != '— Seleziona Cliente —':
                ...
                with col4: kpi_card('Marginalità',           fmt_pct(marg_d),
                                    semaforo(marg_d, soglia_bassa, soglia_alta))
                ...
                for macro in macroprog_drill:
                    ...
                    rows_md.append({
                        ...
                        'Marginalità (%)':       round(mg_md, 1) if not np.isnan(mg_md) else np.nan,
                    })

                df_macro_drill = pd.DataFrame(rows_md)
                if not df_macro_drill.empty:
                    st.dataframe(
                        tabella_semaforo(df_macro_drill, 'Marginalità (%)',
                                         soglia_bassa, soglia_alta),
                        use_container_width=True, hide_index=True
                    )
                ...
                st.markdown(
                    ...
                    f"Costo su venduto: {fmt_eur(cost_d)}  |  Marginalità: **{fmt_pct(marg_d)}**"
                )
                ...
                kpi_df_f = pd.DataFrame([{
                    ...
                    'Marginalità (%)':            round(marg_d, 1) if not np.isnan(marg_d) else None,
                ...
                with col_ef1:
                    xl_f = to_excel({
                        ...
                        'Trend mensile':         df_trend_forn[['Periodo', 'Marginalità (%)',
                    ...
                    st.download_button(
                        'Download Excel',
                        data=xl_f,
                        file_name=f'Marginalita_Forn_{nome_forn.replace(" ","_")}'
                                  f'_{data_da}_{data_a}.xlsx',
                        mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                        key='dl_xl_forn'
                    )
                        ...
                        with st.spinner('Generazione PDF...'):
                            kpi_pdf_f = {
                                ...
                                'Margine su venduto':   (fmt_eur(utile_d),
                                                         semaforo(marg_d, soglia_bassa, soglia_alta)),
                                'Marginalità':          (fmt_pct(marg_d),
                                                         semaforo(marg_d, soglia_bassa, soglia_alta)),
                                'Non ancora fatturato': (fmt_eur(nv_d),   'grigio'),
                            }
                            pdf_bf = genera_pdf(
                                titolo=f'Analisi Marginalità — {nome_forn} → {cli_drill}',
                                periodo_str=periodo_label,
                                kpi_dict=kpi_pdf_f,
